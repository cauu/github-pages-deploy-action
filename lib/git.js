...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield util_1.execute(`git config --global user.name ${constants_1.action.name}`, constants_1.workspace);
            yield util_1.execute(`git config --global user.email ${constants_1.action.email}`, constants_1.workspace);
        }
        catch (error) {
...
function deployToAnotherRepo() {
    return __awaiter(this, void 0, void 0, function* () {
        const temporaryDeploymentDirectory = "gh-action-temp-deployment-folder";
        yield util_1.execute(`git clone ${constants_1.targetRepositoryPath} ${temporaryDeploymentDirectory}`, constants_1.workspace);
        const targetRepoBranchExists = yield util_1.execute(`git ls-remote --heads ${constants_1.targetRepositoryPath} ${constants_1.action.branch} | wc -l`, constants_1.workspace);
        if (!targetRepoBranchExists) {
            /** Create target branch on target repository */
            try {
                console.log("Deployment branch does not exist. Creating....");
                yield util_1.execute(`git switch --orphan ${constants_1.action.branch}`, temporaryDeploymentDirectory);
                yield util_1.execute(`git reset --hard`, temporaryDeploymentDirectory);
                yield util_1.execute(`git commit --allow-empty -m "Initial ${constants_1.action.branch} commit."`, temporaryDeploymentDirectory);
                yield util_1.execute(`git push --force ${constants_1.targetRepositoryPath} ${constants_1.action.branch}`, temporaryDeploymentDirectory);
            }
            catch (error) {
                core.setFailed(`There was an error creating the deployment branch: ${error} on ${constants_1.action.targetRepo} ❌`);
            }
            finally {
                return Promise.resolve(`Deployment branch ${constants_1.action.branch} on ${constants_1.action.targetRepo} creation step complete... ✅`);
            }
        }
    });
}
exports.deployToAnotherRepo = deployToAnotherRepo;
...
function deploy() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        /*
          ...
          */
        if (constants_1.targetRepositoryPath) {
            yield deployToAnotherRepo();
            yield util_1.execute(`git pull origin ${constants_1.action.branch}`, temporaryDeploymentDirectory);
            yield util_1.execute(`git switch ${constants_1.action.branch}`, temporaryDeploymentDirectory);
        }
        else {
        ...
        const executeBuildScript = () => {
            ...
            (constants_1.action.buildScript || []).forEach(script => {
                chain = chain.then(() => util_1.execute(script, constants_1.workspace));
            });
            return chain;
        };
        yield executeBuildScript();
        ...
        if (constants_1.targetRepositoryPath) {
            ...
            yield util_1.execute(`git push --force origin ${constants_1.action.branch}`, 
